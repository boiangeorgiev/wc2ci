...
class WooCommerce {
	...
	public function __construct( $params = array() ) {
		$store_url = isset($params['url']) ? $params['url'] : '';
		$consumer_key = isset($params['key']) ? $params['key'] : '';
		$consumer_secret = isset($params['secret']) ? $params['secret'] : '';
		$is_ssl = isset($params['is_ssl']) ? $params['is_ssl'] : false;
	...
	public function api( $consumer_key, $consumer_secret, $store_url, $is_ssl = false ) {
		if ( ! empty( $consumer_key ) && ! empty( $consumer_secret ) && ! empty( $store_url ) ) {
			...
			throw new Exception( 'Error: api() - Consumer Key / Consumer Secret missing.' );
		} else {
			throw new Exception( 'Error: api() - Store URL missing.' );
		}
	}
